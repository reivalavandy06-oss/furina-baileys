...
const makeSocket = (config) => {
    ...
    if (config.mobile || url.protocol === 'tcp:') {
        throw new boom_1.Boom('Mobile API is not supported anymore', { statusCode: Types_2.DisconnectReason.loggedOut });
    }
    ...
    const sendRawMessage = async (data) => {
        if (!ws.isOpen) {
            throw new boom_1.Boom('Connection Closed', { statusCode: Types_2.DisconnectReason.connectionClosed });
        }
    ...
    const waitForMessage = async (msgId, timeoutMs = defaultQueryTimeoutMs) => {
        ...
        try {
                ...
                onErr = err => {
                    reject(err ||
                        new boom_1.Boom('Connection Closed', {
                            statusCode: Types_2.DisconnectReason.connectionClosed
                        }));
                };
            ...
            if (error instanceof boom_1.Boom && error.output?.statusCode === Types_2.DisconnectReason.timedOut) {
    ...
    const startKeepAliveRequest = () => (keepAliveReq = setInterval(() => {
        ...
        */
        if (diff > keepAliveIntervalMs + 5000) {
            void end(new boom_1.Boom('Connection was lost', { statusCode: Types_2.DisconnectReason.connectionLost }));
        }
        else if (ws.isOpen) {
    ...
    const logout = async (msg) => {
        ...
        void end(new boom_1.Boom(msg || 'Intentional Logout', { statusCode: Types_2.DisconnectReason.loggedOut }));
    };
    ...
    ws.on('close', () => void end(new boom_1.Boom('Connection Terminated', { statusCode: Types_2.DisconnectReason.connectionClosed })));
    // the server terminated the connection
    ws.on('CB:xmlstreamend', () => void end(new boom_1.Boom('Connection Terminated by Server', { statusCode: Types_2.DisconnectReason.connectionClosed })));
    // QR gen
    ws.on('CB:iq,type:set,pair-device', async (stanza) => {
        ...
        const genPairQR = () => {
            ...
            if (!refNode) {
                void end(new boom_1.Boom('QR refs attempts ended', { statusCode: Types_2.DisconnectReason.timedOut }));
                return;
            }
    ...
    ws.on('CB:ib,,downgrade_webclient', () => {
        void end(new boom_1.Boom('Multi-device beta not joined', { statusCode: Types_2.DisconnectReason.multideviceMismatch }));
    });
    ...
    const fetchAccountReachoutTimelock = async () => {
        const queryResult = await (0, mex_js_1.executeWMexQuery)({}, Types_1.QueryIds.REACHOUT_TIMELOCK, Types_2.XWAPaths.xwa2_fetch_account_reachout_timelock, query, generateMessageTag);
        const result = {
    ...
    const fetchNewChatMessageCap = async () => {
        return (0, mex_js_1.executeWMexQuery)({ input: { type: 'INDIVIDUAL_NEW_CHAT_MSG' } }, Types_1.QueryIds.MESSAGE_CAPPING_INFO, Types_2.XWAPaths.xwa2_message_capping_info, query, generateMessageTag);
    };
    return {
        type: 'md',
        ws,
