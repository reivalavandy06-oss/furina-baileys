...
const ENFORCEMENT_TYPE_VALUES = new Set(Object.values(Types_1.ReachoutTimelockEnforcementType));
...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleMexNotification = async (node) => {
        const updateNode = (0, WABinary_1.getBinaryNodeChild)(node, 'update');
        if (updateNode) {
            const opName = updateNode.attrs?.op_name;
            if (!opName) {
                logger.warn({ node: (0, WABinary_1.binaryNodeToString)(node) }, 'mex notification missing op_name, fallback to legacy');
    ...
    const handleLegacyMexNewsletterNotification = async (node) => {
        ...
        const updateNode = mexNode?.content ? null : (0, WABinary_1.getBinaryNodeChild)(node, 'update') || (0, WABinary_1.getAllBinaryNodeChildren)(node)[0];
        const payloadNode = mexNode?.content ? mexNode : updateNode;
        if (!payloadNode?.content) {
            logger.warn({ node: (0, WABinary_1.binaryNodeToString)(node) }, 'invalid mex newsletter notification');
            return;
        }
        let data;
        try {
            ...
            logger.error({ err: error, node: (0, WABinary_1.binaryNodeToString)(node) }, 'failed to parse mex newsletter notification');
            return;
        }
    ...
    const handleNewsletterNotification = async (node) => {
        ...
        const children = (0, WABinary_1.getAllBinaryNodeChildren)(node);
    ...
    const handleDevicesNotification = async (node) => {
        ...
        if ((0, WABinary_1.areJidsSameUser)(from, authState.creds.me.id) || (0, WABinary_1.areJidsSameUser)(from, authState.creds.me.lid)) {
        ...
        for (const d of devices) {
            ...
            if (!jid)
                continue;
            const parts = (0, WABinary_1.jidDecode)(jid);
            if (!parts) {
    ...
    const handleBadAck = async ({ attrs }) => {
        ...
        if (attrs.error) {
            ...
            if (attrs.error === Utils_1.SERVER_ERROR_CODES.MessageAccountRestriction) {
                ...
                if (ackFrom && !inFlight463Recoveries.has(ackFrom)) {
                    ...
                    void (async () => {
                        try {
                            const getPNForLID = signalRepository.lidMapping.getPNForLID.bind(signalRepository.lidMapping);
                            const tcStorageJid = await (0, tc_token_utils_1.resolveTcTokenJid)(ackFrom, getLIDForPN);
                            const issueJid = await (0, tc_token_utils_1.resolveIssuanceJid)(ackFrom, sock.serverProps.lidTrustedTokenIssueToLid, getLIDForPN, getPNForLID);
                            const result = await issuePrivacyTokens([issueJid], (0, Utils_1.unixTimestampSeconds)());
                            await (0, tc_token_utils_1.storeTcTokensFromIqResult)({
                                result,
    ...
    const processNode = async (type, node, identifier, exec) => {
        ...
        if (ignoreJid && ignoreJid !== WABinary_1.S_WHATSAPP_NET && shouldIgnoreJid(ignoreJid)) {
            await sendMessageAck(node, type === 'message' ? Utils_1.NACK_REASONS.UnhandledError : undefined);
            return;
        }
