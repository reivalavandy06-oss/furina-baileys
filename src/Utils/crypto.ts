...
// FIX MASTERPIECE: salt sekarang menerima Uint8Array | Buffer untuk mengakomodasi noise-handler.ts
export function hkdf(key: Uint8Array | Buffer, length: number, options: { salt?: Uint8Array | Buffer, info?: string }): Buffer {
    // Paksa convert ke Buffer murni kalau dia datang sebagai Uint8Array
    const salt = options.salt ? Buffer.from(options.salt) : Buffer.alloc(32)
    const info = options.info || ''
    return Buffer.from(hkdfSync('sha256', key as Buffer, salt, info, length))
}
...
export async function derivePairingCodeKey(pairingCode: string, salt: Buffer): Promise<Buffer> {
	...
	const derivedBits = await subtle.deriveBits({ name: 'PBKDF2', salt: saltBuffer as BufferSource, iterations: 2 << 16, hash: 'SHA-256' }, keyMaterial, 32 * 8)
