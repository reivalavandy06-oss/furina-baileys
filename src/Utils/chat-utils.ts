...
const generateMac = (operation: proto.SyncdMutation.SyncdOperation, data: Uint8Array, keyId: Uint8Array | string, key: Uint8Array) => {
...
export const makeLtHashGenerator = ({ indexValueMap, hash }: Pick<LTHashState, 'hash' | 'indexValueMap'>) => {
	...
	return {
		mix: ({ indexMac, valueMac, operation }: Mac) => {
			...
			if (prevOp) subBuffs.push(prevOp.valueMac as Uint8Array)
		},
		finish: () => {
...
const generatePatchMac = (snapshotMac: Uint8Array, valueMacs: Uint8Array[], version: number, type: WAPatchName, key: Uint8Array) => {
...
export const isMissingKeyError = (error: any): boolean => error?.data?.isMissingKey === true

export const isAppStateSyncIrrecoverable = (error: any, attempts: number): boolean => attempts >= MAX_SYNC_ATTEMPTS || error?.name === 'TypeError'
...
export const decodeSyncdSnapshot = async (name: WAPatchName, snapshot: proto.ISyncdSnapshot, getAppStateSyncKey: FetchAppStateSyncKey, minimumVersionNumber: number | undefined, validateMacs = true, logger?: ILogger) => {
...
export const decodePatches = async (name: WAPatchName, syncds: proto.ISyncdPatch[], initial: LTHashState, getAppStateSyncKey: FetchAppStateSyncKey, options: RequestInit, minimumVersionNumber?: number, logger?: ILogger, validateMacs = true) => {
	const newState: LTHashState = { ...initial, indexValueMap: { ...initial.indexValueMap } }
	const mutationMap: ChatMutationMap = {}
	for (const syncd of syncds) {
		...
		if (validateMacs) {
			...
			const result = mutationKeys(keyEnc.keyData!)
			const computedSnapshotMac = generateSnapshotMac(newState.hash, newState.version, name, result.snapshotMacKey)
			if (Buffer.compare(snapshotMac!, computedSnapshotMac) !== 0) break
		}
		syncd.mutations = []
	}
	return { state: newState, mutationMap }
...
export const chatModificationToAppPatch = (mod: ChatModification, jid: string) => {
	...
	const getMessageRange = (lastMessages: LastMessageList) => {
		...
		if (Array.isArray(lastMessages)) {
			...
			messageRange = { lastMessageTimestamp: lastMsg?.messageTimestamp, messages: lastMessages?.length ? lastMessages.map(m => { if (!m.key?.id || !m.key?.remoteJid) throw new Boom('Incomplete key', { statusCode: 400, data: m }); if (isJidGroup(m.key.remoteJid) && !m.key.fromMe && !m.key.participant) throw new Boom('Expected not from me message to have participant', { statusCode: 400, data: m }); if (!m.messageTimestamp || !toNumber(m.messageTimestamp)) throw new Boom('Missing timestamp in last message list', { statusCode: 400, data: m }); if (m.key.participant) m.key.participant = jidNormalizedUser(m.key.participant); return m }) : undefined }
		} else { messageRange = lastMessages }
		return messageRange
	}
...
export const processSyncAction = (syncAction: ChatMutation, ev: BaileysEventEmitter, me: Contact, initialSyncOpts?: InitialAppStateSyncOptions, logger?: ILogger) => {
	...
	else if (action?.archiveChatAction || type === 'archive' || type === 'unarchive') { const archiveAction = action?.archiveChatAction; const isArchived = archiveAction ? archiveAction.archived : type === 'archive'; const msgRange = !accountSettings?.unarchiveChats ? undefined : archiveAction?.messageRange; ev.emit('chats.update', [{ id, archived: isArchived, conditional: getChatUpdateConditional(id!, msgRange) }]) }
	else if (action?.markChatAsReadAction) { const markReadAction = action.markChatAsReadAction; const isNullUpdate = isInitialSync && markReadAction.read; ev.emit('chats.update', [{ id, unreadCount: isNullUpdate ? null : !!markReadAction?.read ? 0 : -1, conditional: getChatUpdateConditional(id!, markReadAction?.messageRange) }]) }
	...
	else if (action?.contactAction) { const results = processContactAction(action.contactAction, id, logger); emitSyncActionResults(ev, results) }
	...
	else if (action?.labelEditAction) { const { name, color, deleted, predefinedId } = action.labelEditAction; ev.emit('labels.edit', { id: id!, name: name!, color: color!, deleted: deleted!, predefinedId: predefinedId ? String(predefinedId) : undefined }) }
	else if (action?.labelAssociationAction) { ev.emit('labels.association', { type: action.labelAssociationAction.labeled ? 'add' : 'remove', association: type === LabelAssociationType.Chat ? ({ type: LabelAssociationType.Chat, chatId: syncAction.index[2], labelId: syncAction.index[1] } as ChatLabelAssociation) : ({ type: LabelAssociationType.Message, chatId: syncAction.index[2], messageId: syncAction.index[3], labelId: syncAction.index[1] } as MessageLabelAssociation) }) }
	...
	else if (action?.pnForLidChatAction) { if (action.pnForLidChatAction.pnJid) ev.emit('lid-mapping.update', { lid: id!, pn: action.pnForLidChatAction.pnJid }) }
	...
	else if (action?.lockChatAction) { ev.emit('chats.lock', { id: id!, locked: !!action.lockChatAction.locked }) }
	...
	else if (action?.lidContactAction) { ev.emit('contacts.upsert', [{ id: id!, name: action.lidContactAction.fullName || action.lidContactAction.firstName || action.lidContactAction.username || undefined, username: action.lidContactAction.username || undefined, lid: id!, phoneNumber: undefined }]) }
